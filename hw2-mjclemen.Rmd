...
```{r}
crime.offense <- colorFactor("Dark2", crimes$OFFENSE)

leaflet(data = crimes) %>%
  addProviderTiles(providers$Stamen.Toner) %>%
  addPolygons(lng = ~LONGITUDE, lat = ~LATITUDE,
              weight = 2,
              color = ~crime.offense(crimes$OFFENSE),
              label = "Crimes throughout DC",
              highlight = highlightOptions(weight = 3,
                                           color = "blue",
                                           bringToFront = TRUE)) %>%
  addLegend(pal = crime.offense, values = ~crimes$OFFENSE, title = "Color by Offense:",
