...
```{r}
census.diversity.shp <- readOGR("census-diversity.shp", GDAL1_integer64_policy = TRUE)
head(census.diversity.shp@data)

# Create color palette to apply to polygons
color.palette <- colorNumeric("Blues", NULL)

leaflet(data = census.diversity.shp) %>%
  addProviderTiles(providers$Stamen.Toner) %>%
  addPolylines(color = "YlGnBu")
```
