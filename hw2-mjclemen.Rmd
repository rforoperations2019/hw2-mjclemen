...
```{r}
color.by.ccn <- colorQuantile(palette = "YlGnBu", domain = crimes$CCN)

leaflet(data = crimes) %>%
  addTiles() %>%
  addPolylines(lng = ~LONGITUDE, lat = ~LATITUDE,
               color = ~color.by.ccn(CCN),
               weight = 2) %>%
  addLegend(pal = color.by.ccn, values = ~CCN, title = "Color by Criminal Complaint Number:",
  position = "bottomright")

# polyline.map <-  leaflet(x)
# polyline.map <- addTiles(map)
# for(OFFENSE in levels(as.factor(crimes$OFFENSE))){
#   polyline.map <- addPolylines(map, lng=~LONGITUDE,lat=~LATITUDE,data=crimes[crimes$OFFENSE == OFFENSE,], color= ~Color)
# }
# polyline.map
```
# Map with a layer of polygons
```{r}
crime.district <- colorNumeric(palette = c("Blues"), domain = crimes$CENSUS_TRACT)
