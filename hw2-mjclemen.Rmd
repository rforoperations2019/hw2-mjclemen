...
```{r}
crime.district <- colorFactor("Dark2", crimes$DISTRICT)
...
leaflet(data = crimes) %>%
  ...
  addPolygons(lng = ~LONGITUDE, lat = ~LATITUDE,
              weight = 1,
              color = "white",
              fillColor = ~crime.district(crimes$DISTRICT),
              label = "Crimes throughout DC") %>%
  addLegend(pal = crime.district, values = ~crimes$DISTRICT, title = "Color by District:",
